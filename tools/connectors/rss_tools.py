# tools/connectors/rss_tools.py
"""CrewAI tools wrapping rss_monitor.py connector for RSS feed monitoring."""
...
_CONNECTOR_SCRIPT = (
    ...
    / "marketing-skills" / "scripts" / "connectors" / "rss_monitor.py"
)
...
def _run_connector(args: list[str]) -> dict[str, Any] | list:
    ...
    try:
        ...
        if result.returncode != 0:
            logger.warning("rss_monitor.py failed (rc=%d): %s", result.returncode, result.stderr)
        ...
        logger.warning("rss_monitor.py timed out after %ds", CONNECTOR_TIMEOUT_SECONDS)
        return {"error": "timeout"}
    except Exception as exc:
        logger.error("rss_monitor.py exception: %s", exc)
        return {"error": str(exc)}


@tool
def rss_monitor_feed(feed_url: str) -> dict:
    ...
    Returns:
        Dict with entries list (title, link, published, summary) on success.
        Returns {"error": ...} on failure.
    """
    return _run_connector([feed_url])
