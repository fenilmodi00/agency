# tools/connectors/kg_tools.py
"""CrewAI tools wrapping kg.py connector for Wikidata reconciliation."""
...
_CONNECTOR_SCRIPT = (
    ...
    / "marketing-skills" / "scripts" / "connectors" / "kg.py"
)
...
def _run_connector(args: list[str]) -> dict[str, Any] | list:
    ...
    try:
        ...
        if result.returncode != 0:
            logger.warning("kg.py failed (rc=%d): %s", result.returncode, result.stderr)
        ...
        logger.warning("kg.py timed out after %ds", CONNECTOR_TIMEOUT_SECONDS)
        return {"error": "timeout"}
    except Exception as exc:
        logger.error("kg.py exception: %s", exc)
        return {"error": str(exc)}


@tool
def wikidata_reconcile(name: str) -> dict:
    """Reconcile an entity name against Wikidata and return the best QID match.

    Args:
        name: Entity name to reconcile (e.g. 'Apple Inc.').

    Returns:
        Dict with qid, label, description on success.
        Returns {"error": ...} on failure.
    """
    return _run_connector(["reconcile", name])
