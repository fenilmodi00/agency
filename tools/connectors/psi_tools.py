# tools/connectors/psi_tools.py
"""CrewAI tools wrapping psi.py connector for Google PageSpeed Insights."""
...
_CONNECTOR_SCRIPT = (
    ...
    / "marketing-skills" / "scripts" / "connectors" / "psi.py"
)
...
def _run_connector(args: list[str]) -> dict[str, Any] | list:
    ...
    try:
        ...
        if result.returncode != 0:
            logger.warning("psi.py failed (rc=%d): %s", result.returncode, result.stderr)
        ...
        logger.warning("psi.py timed out after %ds", CONNECTOR_TIMEOUT_SECONDS)
        return {"error": "timeout"}
    except Exception as exc:
        logger.error("psi.py exception: %s", exc)
        return {"error": str(exc)}


@tool
def pagespeed_insights(url: str) -> dict:
    """Get Google PageSpeed Insights for a URL (Lighthouse score, FCP, LCP, TBT, CLS).

    Args:
        url: The URL to analyze (e.g. 'https://example.com').

    Returns:
        Dict with lighthouse_score, fcp, lcp, tbt, cls on success.
        Requires PAGESPEED_API_KEY env var. Returns {"error": ...} on failure.
    """
    return _run_connector([url])
