"""Tests for the Launch Registry protocol agent."""

from unittest.mock import patch

import pytest

from agents.protocol.launch_registry import get_launch_registry_agent


class TestLaunchRegistryAgentFactory:
    """Agent factory returns an Agent with the expected configuration."""

    @patch("agents.protocol.launch_registry.get_fireworks_llm")
    @patch("agents.protocol.launch_registry.Agent")
    def test_factory_returns_agent(self, MockAgent, mock_llm):
        """get_launch_registry_agent should create an Agent instance."""
        instance = MockAgent.return_value
        result = get_launch_registry_agent()
