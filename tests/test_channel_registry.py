"""Tests for the Channel Registry protocol agent."""

from unittest.mock import patch

import pytest

from agents.protocol.channel_registry import get_channel_registry_agent


class TestChannelRegistryAgentFactory:
    """Agent factory returns an Agent with the expected configuration."""

    @patch("agents.protocol.channel_registry.get_fireworks_llm")
    @patch("agents.protocol.channel_registry.Agent")
    def test_factory_returns_agent(self, MockAgent, mock_llm):
        """get_channel_registry_agent should create an Agent instance."""
        instance = MockAgent.return_value
        result = get_channel_registry_agent()
