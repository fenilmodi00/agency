"""Tests for the Creator Registry protocol agent."""

from unittest.mock import patch

import pytest

from agents.protocol.creator_registry import get_creator_registry_agent


class TestCreatorRegistryAgentFactory:
    """Agent factory returns an Agent with the expected configuration."""

    @patch("agents.protocol.creator_registry.get_fireworks_llm")
    @patch("agents.protocol.creator_registry.Agent")
    def test_factory_returns_agent(self, MockAgent, mock_llm):
        """get_creator_registry_agent should create an Agent instance."""
        instance = MockAgent.return_value
        result = get_creator_registry_agent()
