"""Tests for the Offer Claims Registry protocol agent."""

from unittest.mock import patch

import pytest

from agents.protocol.offer_claims_registry import get_offer_claims_registry_agent


class TestOfferClaimsRegistryAgentFactory:
    """Agent factory returns an Agent with the expected configuration."""

    @patch("agents.protocol.offer_claims_registry.get_fireworks_llm")
    @patch("agents.protocol.offer_claims_registry.Agent")
    def test_factory_returns_agent(self, MockAgent, mock_llm):
        """get_offer_claims_registry_agent should create an Agent instance."""
        instance = MockAgent.return_value
        result = get_offer_claims_registry_agent()
    ...
    def test_agent_uses_protocol_model(self, MockAgent, mock_llm):
        ...
        get_offer_claims_registry_agent()
